...
const CountryList = () => {
    ...
    const fetchData = async () => {
        try {
            const countryRes = await axios.get('https://snj-global-agency-backend-5uzc.onrender.com/api/admin/visa-countries');
            const categoryRes = await axios.get('https://snj-global-agency-backend-5uzc.onrender.com/api/admin/visa-categories');
    ...
    const handleAddCountry = async (e) => {
        ...
        try {
            await axios.post('https://snj-global-agency-backend-5uzc.onrender.com/api/admin/visa-countries', data, {
    ...
    const handleToggleTop = async (id, currentStatus) => {
        try {
            ...
            await axios.put(`https://snj-global-agency-backend-5uzc.onrender.com/api/admin/visa-countries/toggle-top/${id}`, { is_top: newStatus });
            fetchData();
        } catch (err) { console.error(err); }
    };
    ...
    const toggleStatus = async (country) => {
        ...
        try {
            await axios.put(`https://snj-global-agency-backend-5uzc.onrender.com/api/admin/visa-countries/status/${country.id}`, { status: newStatus });
            fetchData();
        } catch (err) { console.error(err); }
    };
    ...
    const handleDelete = async (id) => {
        Swal.fire({ 
            ...
            if (res.isConfirmed) {
                await axios.delete(`https://snj-global-agency-backend-5uzc.onrender.com/api/admin/visa-countries/${id}`);
                fetchData();
            }
        });
    };
                ...
                {countries.map((country) => (
                    <div key={country.id} className="bg-white group border border-slate-100 hover:shadow-[0_30px_60px_rgba(0,0,0,0.1)] transition-all duration-700 flex flex-col h-full relative overflow-hidden">
                        ...
                        <div className="h-64 w-full relative overflow-hidden shrink-0">
                            {country.image_url ? (
                                <img src={`https://snj-global-agency-backend-5uzc.onrender.com/${country.image_url}`} className="w-full h-full object-cover grayscale-[40%] group-hover:grayscale-0 transition-all duration-1000 group-hover:scale-105" alt={country.country_name} />
                            ) : (
