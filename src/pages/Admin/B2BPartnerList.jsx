...
const B2BPartnerList = () => {
    ...
    const fetchB2B = useCallback(async () => {
        setLoading(true);
        try {
            const res = await axios.get(`https://snj-global-agency-backend-nhxq.onrender.com/api/b2b/list`, {
                params: {
                    page,
                    limit,
                    search: searchTerm,
    ...
    const handleStatus = async (id, newStatus) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await axios.put(`https://snj-global-agency-backend-nhxq.onrender.com/api/b2b/status/${id}`, { status: newStatus });
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await axios.delete(`https://snj-global-agency-backend-nhxq.onrender.com/api/b2b/${id}`);
                fetchB2B();
            } catch (err) {
