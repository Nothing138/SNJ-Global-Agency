...
const FlightRequests = () => {
    ...
    const fetchData = async () => {
        try {
            const res = await axios.get('https://snj-global-agency-backend-5uzc.onrender.com/api/admin/flight-requests');
    ...
    const updateStatus = async (id, status, cost) => {
        try {
            await axios.patch(`https://snj-global-agency-backend-5uzc.onrender.com/api/admin/flight-requests/${id}/status`, { status, total_cost: cost });
            Swal.fire({
                title: 'Success',
