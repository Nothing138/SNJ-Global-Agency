...
const AppliedCandidates = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await axios.get('https://snj-global-agency-backend-5uzc.onrender.com/api/admin/applied-candidates');
    ...
    const updateStatus = async (id, newStatus) => {
        try {
            await axios.patch(`https://snj-global-agency-backend-5uzc.onrender.com/api/admin/applications/${id}/status`, { status: newStatus });
    ...
    const handleDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await axios.delete(`https://snj-global-agency-backend-5uzc.onrender.com/api/admin/applications/${id}`);
                    fetchData();
                ...
                {currentItems.length > 0 ? currentItems.map(app => (
                    <div key={app.id} className="group bg-white border border-slate-100 hover:shadow-[0_20px_50px_rgba(11,31,58,0.08)] transition-all p-8 flex flex-col lg:flex-row items-center justify-between gap-8 relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-2 w-full lg:w-auto justify-end">
                            <a href={`https://snj-global-agency-backend-5uzc.onrender.com/${app.resume_url}`} target="_blank" rel="noreferrer" 
