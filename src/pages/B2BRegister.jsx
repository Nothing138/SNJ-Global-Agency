...
const B2BRegister = () => {
  ...
  const handleSendOtp = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('http://localhost:5000/api/b2b/send-otp', { email: formData.email });
      if (res.data.success) {
        setIsOtpSent(true);
        setTimer(60);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('http://localhost:5000/api/b2b/register', formData);
      if (res.data.success) {
