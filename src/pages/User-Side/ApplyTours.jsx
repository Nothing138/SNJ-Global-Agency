...
const ApplyTours = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        const res = await axios.get(`https://snj-global-agency-backend-5uzc.onrender.com/api/user-travel/user-info/${user.id}`);
        if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('https://snj-global-agency-backend-5uzc.onrender.com/api/user-travel/apply-tour', {
        ...formData,
        user_id: user.id,
        package_id: id
      });
