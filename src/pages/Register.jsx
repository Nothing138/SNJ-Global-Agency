...
const Register = () => {
    ...
    const handleGoogleLogin = () => {
        window.location.href = 'https://snj-global-agency-backend-5uzc.onrender.com/api/auth/google'; // আপনার ব্যাকএন্ডের গুগল পাথ
    };
    ...
    const handleSendOtp = async () => {
        ...
        try {
            ...
            const res = await axios.post('https://snj-global-agency-backend-5uzc.onrender.com/api/verify/send-otp', { email: formData.email });
            if (res.data.success) {
                setIsOtpSent(true);
                setTimer(60);
                Swal.fire({
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await axios.post('https://snj-global-agency-backend-5uzc.onrender.com/api/verify/register', formData);
            if (res.data.success) {
                Swal.fire({ 
                    icon: 'success', 
